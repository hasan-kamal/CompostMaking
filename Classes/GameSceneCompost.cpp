...
void GameSceneCompost::onExit(){
    Layer::onExit();
}

Scene* GameSceneCompost::createScene()
{
    // 'scene' is an autorelease object
    auto scene = Scene::create();
    
    // 'layer' is an autorelease object
    auto layer = GameSceneCompost::create();
...
bool GameSceneCompost::is_vector_within_sprite(Sprite *s, Vec2 vector){
    if(s==nullptr)
        return false;
    
    double x1 = s->getPosition().x - s->getContentSize().width*s->getScaleX()/2;
    double x2 = s->getPosition().x + s->getContentSize().width*s->getScaleX()/2;
    double y1 = s->getPosition().y - s->getContentSize().height*s->getScaleY()/2;
    double y2 = s->getPosition().y + s->getContentSize().height*s->getScaleY()/2;
...
bool GameSceneCompost::init()
{
    ...
    this->addChild(background_sprite, -3);
    ...
    auto closeItem = MenuItemImage::create(
                                           "left_angle.png",
                                           "left_angle.png",
                                           CC_CALLBACK_1(GameSceneCompost::back_clicked, this));
    ...
    auto balloonItem = MenuItemImage::create(
                                             ...
                                             "balloon_icon_scaled.png",
                                             CC_CALLBACK_1(GameSceneCompost::start_reward, this));
    ...
    auto menu_level_selection = MenuItemImage::create("menu_button.png", "menu_button.png", CC_CALLBACK_1(GameSceneCompost::menu_level_select, this));
    ...
    float paddingBetween = visibleSize.width*8.0/100;
    
    //brown pot
    brown_pot_fg = Sprite::create("brown_pot_foreground.png");
    brown_pot_fg->setScale(visibleSize.height*35.0/100.0/brown_pot_fg->getContentSize().height);
    float width = brown_pot_fg->getContentSize().width*brown_pot_fg->getScale();
    float height = brown_pot_fg->getContentSize().height*brown_pot_fg->getScale();
    brown_pot_fg->setAnchorPoint(Vec2(0.5, 0.5));
    brown_pot_fg->setPosition(Vec2(origin.x + visibleSize.width/2.0 - paddingBetween - width/2.0, origin.y + visibleSize.height*32.0/100.0));
    this->addChild(brown_pot_fg, 1);
    
    brown_pot_bg = Sprite::create("brown_pot_background.png");
    brown_pot_bg->setScale(visibleSize.height*35.0/100.0/brown_pot_bg->getContentSize().height);
    brown_pot_bg->setAnchorPoint(Vec2(0.5, 0.5));
    brown_pot_bg->setPosition(Vec2(origin.x + visibleSize.width/2.0 - paddingBetween - width/2.0, origin.y + visibleSize.height*32.0/100.0));
    this->addChild(brown_pot_bg, -1);
    
    //green tray, its span denotes selection area for brown_pot
    green_tray = Sprite::create("green_tray_bright.png");
    green_tray->setScale(width*80.0/100.0/green_tray->getContentSize().width, visibleSize.height*20.0/100.0/green_tray->getContentSize().height);
    green_tray->setAnchorPoint(Vec2(0.5, 0.5));
    green_tray->setPosition(Vec2(brown_pot_fg->getPosition().x, brown_pot_fg->getPosition().y + height/2.0));
    green_tray->setVisible(false);
    this->addChild(green_tray, -2);
    cout<<"green.x "<<green_tray->getPosition().x<<" green.y "<<green_tray->getPosition().y<<endl;
    
    //blue tray
    blue_tray = Sprite::create("blue_tray.png");
    blue_tray->setScale(visibleSize.height*30.0/100.0/blue_tray->getContentSize().height);
    blue_tray->setAnchorPoint(Vec2(0.5, 0.5));
    blue_tray->setPosition(Vec2(origin.x + visibleSize.width/2 + paddingBetween + width/2, origin.y + visibleSize.height*32.0/100));
    this->addChild(blue_tray);
    cout<<"blue.x "<<blue_tray->getPosition().x<<" blue.y "<<blue_tray->getPosition().y<<endl;
    
    //touch listener
    auto touchEventListener = EventListenerTouchOneByOne::create();
    touchEventListener->onTouchBegan = CC_CALLBACK_2(GameSceneCompost::onTouchBegan, this);
    touchEventListener->onTouchMoved = CC_CALLBACK_2(GameSceneCompost::onTouchMoved, this);
    touchEventListener->onTouchEnded = CC_CALLBACK_2(GameSceneCompost::onTouchEnded, this);
    ...
    for(objCompost o : LevelCompost::objects){
        if(o.level_number==1){
            string s = o.image_path;
            auto sprite = Sprite::create(s);
            
            entity e{sprite, o.can_be_put_in_compost, o.hint_sound_path_english, o.hint_sound_path_hindi};
    ...
    //LevelSelectionLayer
    level_selection_layer = LevelSelectionLayerCompost::create(this);
...
void GameSceneCompost::update(float dt){
...
bool GameSceneCompost::onTouchBegan(Touch *touch, Event *unused_event){
...
void GameSceneCompost::onTouchMoved(Touch *touch, Event *unused_event){
...
void GameSceneCompost::sprite_border_reposition(){
...
void GameSceneCompost::onTouchEnded(Touch *touch, Event *unused_event){
    
    if(!touched_sprite_initially)
        return;
    
    if(is_vector_within_sprite(green_tray, touch->getLocation())||is_vector_within_sprite(brown_pot_fg, touch->getLocation())){
...
void  GameSceneCompost::sprite_reposition(){
...
void GameSceneCompost::correct_answer(){
    ...
    auto destination_pos = current_display_object.is_biodegradable?brown_pot_fg->getPosition():blue_tray->getPosition();
    auto action_move_to = MoveTo::create(0.3f, destination_pos);
    auto ease_move = EaseIn::create(action_move_to, 5.0f);
    auto scale_down = ScaleTo::create(0.3f, brown_pot_bg->getContentSize().height*8/100/current_display_object.sprite->getContentSize().height);
    current_display_object.sprite->runAction(ease_move);
...
void GameSceneCompost::wrong_answer(){
...
void GameSceneCompost::set_object_display(entity obj_to_disp){
    auto origin = Director::getInstance()->getVisibleOrigin();
    auto visibleSize = Director::getInstance()->getVisibleSize();
...
void GameSceneCompost::next_level(){
...
void GameSceneCompost::switch_level(int new_level_index){
...
void GameSceneCompost::restart_level(){
...
void GameSceneCompost::update_stars(){
    auto origin = Director::getInstance()->getVisibleOrigin();
    auto visibleSize = Director::getInstance()->getVisibleSize();
...
void GameSceneCompost::back_clicked(cocos2d::Ref *pSender){
    CocosDenshion::SimpleAudioEngine::getInstance()->playEffect(GlobalSettings::click.c_str());
...
void GameSceneCompost::start_reward(cocos2d::Ref *pSender){
    CocosDenshion::SimpleAudioEngine::getInstance()->playEffect(GlobalSettings::click.c_str());
    auto reward_scene = RewardSelectionScene::createScene();
    Director::getInstance()->pushScene(TransitionFade::create(0.5f, reward_scene, Color3B(Colors::clouds)));
}

void GameSceneCompost::menu_level_select(cocos2d::Ref *pSender){
    CocosDenshion::SimpleAudioEngine::getInstance()->playEffect(GlobalSettings::click.c_str());
    level_selection_layer->show();
}

void GameSceneCompost::shuffle_unanswered_objects(){
